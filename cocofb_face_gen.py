...
parser.add_argument('--year', default=2014, type=int, help='2017 or 2014')
...
def load_dataset(ann_folder):
    train_ann_file = ann_folder + f"keypoints_train{args.year}.json"
    val_ann_file = ann_folder + f"keypoints_val{args.year}.json"
    cocofb_train = COCO(train_ann_file)
    cocofb_val = COCO(val_ann_file)
    return {"train":cocofb_train, "val":cocofb_val}
...
if __name__ == '__main__':
    ...
    cocofbs = load_dataset(ann_folder)

    _t = {'forward_pass': Timer(), 'misc': Timer()}

    # testing begin
    print(len(cocofbs['train'].dataset['annotations']))
    print(len(cocofbs['val'].dataset['annotations']))
    ann_id = len(cocofbs['train'].dataset['annotations']) + len(cocofbs['val'].dataset['annotations'])
    print(ann_id)
    for key in cocofbs.keys():
        cocofb = cocofbs[key]
        cocofb.info()
        for i, ann_image in enumerate(tqdm(cocofb.dataset['images'], desc=f"[Image Detection]{key.title()} Set")):
            image_folder = args.dataset_folder + "images/" + f"{key}{args.year}/"
            ...
            if args.save_image:
                ...
                # save image
                if not os.path.exists(f"./results/{key}{args.year}"):
                    os.makedirs(f"./results/{key}{args.year}")
                name = f"./results/{key}{args.year}/" + str(i) + ".jpg"
                cv2.imwrite(name, img_raw)

        for ann in cocofb.dataset['annotations']:
            for b in ann['bbox']:
                if b < 0: raise ValueError("All bounding boxes should have positive coordinates.")

        with open(ann_folder+f'landmarks_{key}{args.year}.json', 'w+') as fw:
